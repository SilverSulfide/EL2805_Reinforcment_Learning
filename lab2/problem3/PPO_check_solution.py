...
PPO.actor_network.load_state_dict(checkpoint)
PPO.critic_network.load_state_dict(checkpoint2)
...
for i in EPISODES:
    ...
    while not done:
        ...
        # FIXME: Swap back to sampling
        mu, var = PPO.actor_network.inference(torch.tensor([state]).to(device))
